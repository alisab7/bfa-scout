"""
Apply phase_7_nt_role.sql against the live DB. Same transaction
semantics as _dryrun_7.py — but COMMITs on success.

The migration's pre-flight + audit DO-blocks RAISE on any drift,
which propagates through psycopg2 and triggers the rollback path.
"""
import os
import sys
...
sql = sql.replace("COMMIT;", "-- COMMIT; (issued by _apply_7.py after audit pass)")
...
try:
    ...
    conn.commit()
    print("\nCOMMIT OK -- Phase 7 NT role + created_by_role is live.")
